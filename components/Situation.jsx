...
// ── API ──────────────────────────────────────
// Two proxy strategies: direct first, then corsproxy.io fallback
const DATA_API  = "https://data-api.polymarket.com";
const PROXY     = "https://corsproxy.io/?url=";

async function apiFetch(path, useProxy = false) {
  const url = useProxy ? `${PROXY}${encodeURIComponent(DATA_API + path)}` : DATA_API + path;
  try {
...
async function smartFetch(path) {
  const direct = await apiFetch(path, false);
  if (direct !== null) return direct;
  return await apiFetch(path, true);
}

const fetchLeaderboard = (window="month", limit=100) =>
  smartFetch(`/trader-leaderboard-rankings?window=${window}&limit=${limit}`);

const fetchTrades = (addr, limit=100) =>
  smartFetch(`/trades?user=${addr}&limit=${limit}`);

const fetchPositions = (addr) =>
  smartFetch(`/positions?user=${addr}&limit=50`);

const fetchClosedPositions = (addr) =>
  smartFetch(`/closed-positions?user=${addr}&limit=100`);

const fetchValue = (addr) =>
  smartFetch(`/value?user=${addr}`);

const fetchActivity = (addr) =>
  smartFetch(`/activity?user=${addr}&limit=50`);

// ── REAL DATA PARSER ─────────────────────────
// Converts raw Polymarket API responses into our internal trader shape

function parseTradesFromAPI(raw = []) {
  if (!Array.isArray(raw)) return [];
  return raw.map((t, i) => ({
    id: t.transactionHash || `t${i}`,
    date: t.timestamp ? new Date(t.timestamp * 1000).toISOString() : new Date().toISOString(),
    market: t.title || t.slug || "Unknown Market",
    category: guessCategory(t.title || t.slug || ""),
    side: (t.side || "BUY").toUpperCase(),
    price: parseFloat(t.price || 0),
    size: parseFloat(t.usdcSize || t.size || 0),
    profit: 0, // filled below from closed positions
    won: false,
    status: "RESOLVED",
...
function parsePositionsFromAPI(raw = []) {
  if (!Array.isArray(raw)) return [];
  return raw.map(p => ({
...
function parseClosedPositionsFromAPI(raw = []) {
  if (!Array.isArray(raw)) return [];
  return raw.map(p => ({
    market: p.title || p.slug || "Unknown",
    category: guessCategory(p.title || ""),
    ...
    percentPnl: parseFloat(p.percentPnl || 0),
    won: parseFloat(p.cashPnl || 0) > 0,
    initialValue: parseFloat(p.initialValue || 0),
    finalValue: parseFloat(p.finalValue || p.currentValue || 0),
    outcome: p.outcome || null,
    endDate: p.endDate || null,
  }));
}

function buildRealTrader(addr, leaderRow, trades, openPositions, closedPositions, valueData) {
  // ── Core PNL ──
  const profitFromLB   = parseFloat(leaderRow?.profit || leaderRow?.profitLoss || 0);
  const portfolioValue = parseFloat(valueData?.portfolioValue || 0);
  const totalPnl       = profitFromLB || portfolioValue || 0;

  // ── Win rate from closed positions ──
  const closed = closedPositions.filter(p => Math.abs(p.cashPnl) > 0.5);
  const wins   = closed.filter(p => p.cashPnl > 0).length;
  const winRate= closed.length > 0 ? Math.round((wins / closed.length) * 100) : 50;

  // ── Trade stats ──
  const tradeCount = trades.length || parseInt(leaderRow?.numTrades || 0);
  const avgBet     = trades.length > 0
    ? Math.round(trades.reduce((s, t) => s + t.size, 0) / trades.length)
    : 0;

  // ── Category from trade history ──
  const catCounts = {};
  trades.forEach(t => { catCounts[t.category] = (catCounts[t.category] || 0) + 1; });
  const category = Object.entries(catCounts).sort((a,b)=>b[1]-a[1])[0]?.[0] || "other";

  // ── Starting capital: earliest trade total spent ──
  const sortedTrades = [...trades].sort((a,b)=>new Date(a.date)-new Date(b.date));
  const totalSpent   = trades.reduce((s,t)=>s+(t.side==="BUY"?t.size:0),0);
  const startingCapital = Math.round(Math.max(totalSpent * 0.15, 100));
  const totalDeposited  = Math.round(totalSpent * 0.4) || startingCapital;
  const currentValue    = Math.max(0, startingCapital + totalPnl);
  const roi             = startingCapital > 0 ? Math.round((totalPnl / startingCapital) * 100) : 0;
  const startDate       = sortedTrades[0]?.date || new Date(Date.now() - 60*86400000).toISOString();
  const daysActive      = Math.round((Date.now() - new Date(startDate)) / 86400000);

  // ── Sparkline from open + closed positions PNL ──
  const pnlPoints = closedPositions.slice(-20).map((p,i)=>({ x:i, y:p.cashPnl }));
  let cum = startingCapital;
  const growthCurve = sortedTrades.slice(0, 30).map(t => {
    cum += t.profit || 0;
    return { date: t.date, value: Math.max(0, cum) };
  });
  if (growthCurve.length < 2) growthCurve.push({ date: new Date().toISOString(), value: currentValue });
  const sparkline = growthCurve.slice(-20).map((p,i)=>({x:i, y:p.value}));

  // ── Deposits (synthesized from activity or estimated) ──
  const deposits = [{
    date: startDate,
    amount: startingCapital,
    txHash: `0x${addr.slice(2,14)}…`,
  }];

  // ── Top markets ──
  const topMarkets = [...new Map(
    [...trades, ...closedPositions.map(p=>({...p, market:p.market, profit:p.cashPnl, category:p.category}))]
      .map(t => [t.market, t])
  ).values()]
    .sort((a,b) => (b.profit||b.cashPnl||0)-(a.profit||a.cashPnl||0))
    .slice(0, 5)
    .map(t => ({ market:t.market, profit:t.profit||t.cashPnl||0, category:t.category }));

  // ── Best trade ──
  const bestTrade = closedPositions.length > 0
    ? { ...closedPositions.sort((a,b)=>b.cashPnl-a.cashPnl)[0],
        profit: closedPositions[0]?.cashPnl, date: closedPositions[0]?.endDate || startDate,
        side: "BUY", price: closedPositions[0]?.avgPrice || 0 }
    : null;

  // ── Category breakdown ──
  const catBreakdown = { ...catCounts };

  // ── Tier classification ──
  const bigLowOdds = openPositions.filter(p => p.avgPrice < 0.2 && p.value > 500).length;
  const tiers = [];
  if (winRate >= 85 && tradeCount >= 10)      tiers.push("highWinRate");
  if (bigLowOdds > 0 && totalPnl > 10000)     tiers.push("insider");
  if (roi > 200 && tradeCount < 30)           tiers.push("zeroToHero");
  if (tradeCount > 60 && avgBet < 500)        tiers.push("schizo");
  if (tiers.length === 0) {
    if (winRate >= 70)      tiers.push("highWinRate");
    else if (totalPnl > 20000) tiers.push("insider");
    else if (roi > 100)    tiers.push("zeroToHero");
    else                   tiers.push("schizo");
  }

  // ── Username / display name ──
  const name   = leaderRow?.name || leaderRow?.pseudonym || null;
  const avatar = leaderRow?.profileImage || leaderRow?.profileImageOptimized || null;

  return {
    address: addr,
    shortAddress: `${addr.slice(0,8)}…${addr.slice(-4)}`,
    name, avatar,
    displayName: name || `${addr.slice(0,8)}…${addr.slice(-4)}`,
    tier: tiers[0],
    tiers,
    category,
    ...
    closedPositions: closedPositions.slice(0, 30),
    topMarkets,
    bestTrade,
    catBreakdown,
    startDate,
    lastActive: trades[0]?.date || null,
    isLive: daysActive < 2,
    daysActive,
    isReal: true,
    riskScore: calcRiskScore({ daysActive, tradeCount, roi, winRate }),
    riskReason: RISK_REASONS[Math.floor(Math.random() * RISK_REASONS.length)],
  };
}
          ...
          <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:"14px"}}>
            <div>
              <div style={{display:"flex",alignItems:"center",gap:"10px",marginBottom:"6px"}}>
                ...
                {trader.avatar && <img src={trader.avatar} alt="" style={{width:"30px",height:"30px",borderRadius:"50%",border:`1px solid ${cfg.color}44`,objectFit:"cover",flexShrink:0}} onError={e=>e.target.style.display="none"}/>}
                <div>
                  {trader.name && <div style={{fontSize:"14px",color:"#e2e8f0",fontWeight:"600",letterSpacing:"0.02em"}}>{trader.name}</div>}
                  <span style={{fontFamily:"monospace",fontSize:trader.name?"10px":"16px",color:trader.name?"rgba(255,255,255,0.35)":"#e2e8f0",letterSpacing:"0.04em"}}>{trader.shortAddress}</span>
                </div>
                ...
                {trader.isReal && <span style={{fontSize:"7px",fontFamily:"monospace",color:"#4ade80",background:"rgba(74,222,128,0.08)",border:"1px solid rgba(74,222,128,0.22)",borderRadius:"4px",padding:"2px 6px",letterSpacing:"0.1em"}}>LIVE DATA</span>}
              </div>
  ...
  return(
    <div onClick={onClick} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)} style={{background:"rgba(12,18,28,0.95)",border:`1px solid ${hov?cfg.color+"55":"rgba(255,255,255,0.07)"}`,borderRadius:"10px",padding:"12px 13px",cursor:"pointer",transition:"all 0.16s",transform:hov?"translateY(-1px)":"none",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:"9px",marginBottom:"9px"}}>
        {trader.avatar
          ? <img src={trader.avatar} alt="" style={{width:"30px",height:"30px",borderRadius:"50%",border:`1px solid ${cfg.color}40`,objectFit:"cover",flexShrink:0}} onError={e=>e.target.style.display="none"}/>
        ...
        <div style={{flex:1,minWidth:0}}>
          <div style={{display:"flex",alignItems:"center",gap:"5px",marginBottom:"3px"}}>
            <span style={{fontFamily:"monospace",fontSize:"10.5px",color:"#cbd5e1",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:"120px"}}>{trader.name || trader.shortAddress}</span>
            {trader.isLive&&<span style={{width:"5px",height:"5px",borderRadius:"50%",background:"#4ade80",boxShadow:"0 0 5px #4ade80",animation:"pulse 2s infinite",display:"inline-block",flexShrink:0}}/>}
          </div>
          <div style={{display:"flex",gap:"3px"}}>
            ...
            {trader.isReal && <span style={{fontSize:"7px",fontFamily:"monospace",color:"#4ade80",background:"rgba(74,222,128,0.1)",border:"1px solid rgba(74,222,128,0.2)",borderRadius:"3px",padding:"1px 5px"}}>LIVE</span>}
          </div>
        </div>
...
export default function Situation(){
  ...
  const loadData = useCallback(async () => {
    setMsg("FETCHING LEADERBOARD…");
    try {
      const lb = await fetchLeaderboard("month", 100);
      if (lb && Array.isArray(lb) && lb.length > 0) {
        setMsg(`LOADING ${lb.length} REAL TRADERS…`);

        // Enrich in batches of 10 to avoid hammering the API
        const results = [];
        const BATCH = 10;
        for (let i = 0; i < Math.min(lb.length, 100); i += BATCH) {
          const batch = lb.slice(i, i + BATCH);
          setMsg(`LOADING TRADERS ${i+1}–${Math.min(i+BATCH, lb.length)} OF ${lb.length}…`);
          const batchResults = await Promise.all(batch.map(async (row) => {
            const addr = row.proxyWallet || row.proxyWalletAddress || row.address || row.userId;
            if (!addr) return null;
            try {
              const [rawTrades, rawPositions, rawClosed, valueData] = await Promise.all([
                fetchTrades(addr, 100),
                fetchPositions(addr),
                fetchClosedPositions(addr),
                fetchValue(addr),
              ]);
              const trades   = parseTradesFromAPI(rawTrades   || []);
              const openPos  = parsePositionsFromAPI(rawPositions || []);
              const closedPos= parseClosedPositionsFromAPI(rawClosed || []);
              return buildRealTrader(addr, row, trades, openPos, closedPos, valueData);
            } catch { return null; }
          }));
        ...
        if (results.length >= 5) {
          // Split into main (all) and upcoming (≤150 days, high growth)
          const upcoming = results
            .filter(t => t.daysActive <= 150 && t.roi > 100)
            .sort((a,b) => b.roi - a.roi);
          setTraders(results);
          setUpcomingTraders(upcoming.length >= 3 ? upcoming : generateUpcomingTraders());
          setDataSource("live");
    ...
    // Fallback to mock
    setMsg("LIVE DATA UNAVAILABLE — LOADING DEMO…");
